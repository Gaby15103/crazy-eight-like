TODO:
 - Make the program stop more easily without the game ending. &check;
 - Starting Player should be random. &check;
 - stacking two to draw 2 other card
 - changing strategy when on player as only one card left...
 - calculating the position of each player at the end of the game.
 - Logging the action done by some specific card. &check;